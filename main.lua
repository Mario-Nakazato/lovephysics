require "palavras" -- Mudar nome
require "camera"
require "mundo"
require "objeto" -- Nome muito generico mudar para ser mais expecifico.
...
function love.load(arg)
  ...
  heptagono = objeto.novo("line", terra, 128 * 7, 0, "dynamic", 0, 0, 64, 64, 128, 64, 192, 0, 160, -32, 96, -64, 32, -32)
  table.insert(z, heptagono.draw)
  
  octogono = objeto.novo("line", terra, 128 * 8, 0, "dynamic", 0, 0, 16, 48, 64, 64, 128, 64, 192, 0, 128, -64, 64, -64, 16, -48)
...
function love.keypressed(tecla, cod, repeticao)
  ...
  if tecla == "f1" then -- Tecla para ativar ou desativar o love.update.
    update = not update
  end
  if tecla == "f5" then -- Tecla para reiniciar love.load().
    love.load(arg)
  end
  ...
  if tecla == "kp."then -- "tecla do numerico , "
...
function inicioContato(a, b, contato) -- Função deve ser criada para cada mundo ! .
  
  
  
end

function fimContato(a, b, contato) -- Função deve ser criada para cada mundo ! .
  
  
  
end

function preContato(a, b, contato) -- Função deve ser criada para cada mundo ! .
...
function posContato(a, b, contato, normalImpulso, tangenteImpulso) -- Função deve ser criada para cada mundo ! .
